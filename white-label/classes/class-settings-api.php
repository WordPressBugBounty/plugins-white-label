...
{ // phpcs:ignore
    ...
    /**
     * Displays subheading text for settings.
     *
     *   @param array $args settings field args.
     */
    ...
    /**
     * Displays grouping text for settings.
     *
     *   @param array $args settings field args.
     */
    public function callback_grouping($args)
    {
        $html = '<h4 class="white-label-grouping">'.$args['name'].'</h4>';
        $html .= $this->get_field_description($args);
        echo $html;
    }
    ...
    public function callback_dashboard_widgets($args)
    {
        ...
        $html .= '<input type="checkbox" class="checkbox white-label-select-all" />';
        $html .= __('Remove', 'white-label');
        $html .= '</td>';
        ...
        foreach ($args['options'] as $key => $label) {
            ...
            $html .= '<tr>';
            $html .= '<td>'.sprintf('<input type="checkbox" class="checkbox" id="white-label-%1$s[%2$s][%3$s]" name="%1$s[%2$s][%3$s]" %4$s />', $args['section'], $args['id'], $key, checked($checked, 'on', false)).'</td>';
    ...
    /**
     * Displays a table for Elementor editor navigation settings.
     *
     * @param array $args settings field args.
     */
    public function callback_elementor_editor_navigation($args)
    {
        $html = '';

        $elementor_editor_navigation = $this->get_option($args['id'], $args['section'], $args['std']);
        $html .= sprintf('<input type="hidden" name="%1$s[%2$s]" value="" />', $args['section'], $args['id']);
        ...
        $html .= '<td style="width:10%;">';
        $html .= '<input type="checkbox" class="checkbox white-label-select-all" />';
        $html .= __('Hide', 'white-label');
        $html .= '</td>';
        $html .= '<td style="width:90%;">'.__('Menu Item', 'white-label').'</td>';
        $html .= '</tr>';
        $html .= '</thead>';

        $html .= '<tbody>';
        $wl_elementor_editor_navigation_counter = 0;

        foreach ($args['options'] as $key => $name) {
            $wl_elementor_editor_navigation_counter++;
            $checked = isset($elementor_editor_navigation[$key]) ? $elementor_editor_navigation[$key] : '0';
    ...
    public function callback_sidebar_menus($args)
    {
        ...
        $html .= '<td style="width:10%;">';
        $html .= '<input type="checkbox" class="checkbox white-label-select-all" />';
        $html .= __('Hide', 'white-label').'<a target="_blank" tabindex="-1" class="white-label-help" href="https://whitewp.com/documentation/article/hide-wordpress-admin-menus/"><span class="dashicons dashicons-editor-help"></span></a>';
        $html .= '</td>';
        ...
        foreach ($args['options'] as $key => $label) {
            ...
            $html .= '<tr>';
            $html .= '<td>'.sprintf('<input type="checkbox" class="checkbox" id="wl-hidden-%1$s[%2$s][%3$s][%4$s]" name="%1$s[%2$s][%3$s][%4$s]" value="%4$s" %5$s />', $args['section'], 'hidden_sidebar_menus', 'parents', $key, checked($checked, $key, false)).'</td>';
            ...
            if ($submenus && is_array($submenus)) {
                foreach ($submenus as $slug => $name) {
                    ...
                    $html .= '<tr>';
                    $html .= '<td>'.sprintf('<input type="checkbox" class="checkbox" id="wl-hidden-%1$s[%2$s][%3$s][%4$s][%5$s]" name="%1$s[%2$s][%3$s][%4$s][%5$s]" value="%5$s" %6$s />', $args['section'], 'hidden_sidebar_menus', 'children', $parent, $slug, checked($checked, $slug, false)).'</td>';
